...
use crate::{
    model::{common::identity::ComponentId, CombinedMetadata, LinkComponent},
    store::{
        api::{anchor::ApiDataAnchor, ApiData},
        code::{anchor::CodeDataAnchor, item::CodeItem, CodeData},
        combined::{anchor::CombinedAnchor, Combined},
    },
};
...
impl CombinedOriginApis {
    ...
    pub fn get_origin(&self, key: &str) -> Option<&str> {
        ...
        let key = match self.key_hashes.get(key) {
            Some(key) => key,
            None => return None,
        };
